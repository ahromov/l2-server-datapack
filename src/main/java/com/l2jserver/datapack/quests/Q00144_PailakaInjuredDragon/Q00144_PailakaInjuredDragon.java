...
import com.l2jserver.gameserver.util.Rnd;
...
public class Q00144_PailakaInjuredDragon extends Quest {
	...
	// Arrays
	private static final int[] NPCS = { KETRA_ORC_SHAMAN, KETRA_ORC_SUPPORTER, KETRA_ORC_INTELIGENCE_OFFICER,
	...
	public final String onKill(L2Npc npc, L2PcInstance player, boolean isSummon) {
		...
		// There are lots of mobs walls, and item get is random, it could happen that
		// you dont get the item until the last wall, and there's 4 different silenos
		// groups. 1 enchant comes only from group 2 and the 2nd comes from group 4.
		// Chances, lets say 20% of getting the enchant when killing the
		// right mob When you kill a mob wall, another mage type appears behind. If all
		// mobs from the front are killed then the ones that are behind are despawned.
		// Also this mobs should be damaged, like with 30% of max HP, because they
	...
	public String onSpawn(L2Npc npc) {
		if (npc instanceof L2MonsterInstance) {
			for (int mobId : WALL_MONSTERS) {
				// Every monster on pailaka should be Aggresive and Active, with the same clan,
				// also wall mobs cannot move, they all use magic from far, and if you get in
				// combat range they hit.
				if (mobId == npc.getId()) {
	...
	public String onEnterZone(L2Character character, L2ZoneType zone) {
		if ((character instanceof L2PcInstance) && !character.isDead() && !character.isTeleporting()
			...
			if ((world != null) && (world.getTemplateId() == INSTANCE_ID)) {
				// If a player wants to go by a mob wall without kill it, he will be returned
	...
	// Spawns Mage Type silenos behind the one that was killed. Aggro against the
	// player that kill the mob.
	...
	// This function will check if there is other mob alive in this wall of mobs. If
	// all mobs in the first row are dead then despawn the second row mobs, the
	// mages.
	...
	private final synchronized void enterInstance(L2PcInstance player) {
		...
		if (world != null) {
			if (world.getTemplateId() != INSTANCE_ID) {
				player.sendPacket(
						SystemMessageId.YOU_HAVE_ENTERED_ANOTHER_INSTANT_ZONE_THEREFORE_YOU_CANNOT_ENTER_CORRESPONDING_DUNGEON);
				return;
			}
