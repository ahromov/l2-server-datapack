...
public class TeleportBoard implements IParseBoardHandler
{
	...
	private String showSavedPoints(L2PcInstance activeChar)
	{
			...
			PreparedStatement st = con.prepareStatement("SELECT * FROM comteleport WHERE charId=?;");)
		{
			...
			while (rs.next())
			{
				...
				html.append("<button value=\"" + tp.TpName + "\" action=\"bypass -h _bbsteleport;teleport; " + tp.xC
					+ " " + tp.yC + " " + tp.zC + " " + 100000
					+ "\" width=130 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
				html.append("</td>");
				html.append("<td align=center>");
				html.append("<button value=\"Delete\" action=\"bypass -h _bbsteleport;delete;" + tp.TpId
	...
	private void deletePoint(L2PcInstance activeChar, int TpNameDell)
	{
		try (Connection conDel = ConnectionFactory.getInstance().getConnection();
			PreparedStatement stDel = conDel
			...
			PreparedStatement st = con.prepareStatement("SELECT COUNT(*) FROM comteleport WHERE charId=?;");)
		{
			...
			if (rs.getInt(1) <= 9)
			{
					...
					.prepareStatement("SELECT COUNT(*) FROM comteleport WHERE charId=? AND name=?;");)
				{
					...
					if (rs1.getInt(1) == 0)
					{
						try (PreparedStatement stAdd = con.prepareStatement(
